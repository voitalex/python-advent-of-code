...
def first_task(directions: Iterable[str]) -> int:
    ...
    exactly one house to the north (^), south (v), east (>), or west (<). After each move,
    ...
    houses = set(itertools.accumulate(itertools.chain.from_iterable(directions), _move, initial=Position(x=0, y=0)))
...
def second_task(directions: Iterable[str]) -> int:
    ...
    * ^v^v^v^v^v now delivers presents to 11 houses, with Santa going one direction and Robo-Santa going the other."
    ...
    santa_directions, robot_directions = itertools.tee(itertools.chain.from_iterable(directions))

    santa_houses = set(
        itertools.accumulate(
            (direction for index, direction in enumerate(santa_directions) if index % 2 == 0),
            _move,
    ...
    robot_houses = set(
        itertools.accumulate(
            (direction for index, direction in enumerate(robot_directions) if index % 2 == 1),
            _move,
            initial=Position(x=0, y=0),
        )
    )
