...
class BoardSet:
    ...
    def __init__(self, line_size: int, numbers: Iterable[str]) -> None:
        self._boards: List[Board] = [Board(index, values) for index, values in enumerate(chunked(numbers, line_size))]
...
def first_task(strings: Iterable[str]):
    ...
    for number in numbers:
        winner = first(board_set.mark(number), default=None)
...
def second_task(strings: Iterable[str]) -> int:
    ...
    winning_score = 0
    for number in numbers:
        winner = last(board_set.mark(number), default=None)
