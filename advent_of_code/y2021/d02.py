...
class Command:
    """ Описание команды на изменение курса """
    action: str
    offset: int
...
def _parse(command: str) -> Command:
    """ Возвращает команду на изменение курса исходя из строкового представления """

    template = re.compile(r'(down|forward|up).(\d+)')
    if res := re.search(template, command):
        return Command(action=res.group(1), offset=int(res.group(2)))
...
def first_task(strings: Iterable[str]) -> int:
    ...
    def _move(current: Point, command: Command):
        """ Перемещение подводной лодки """

        forward_offset = depth_offset = 0
        if command.action == 'forward':
            forward_offset = command.offset
        elif command.action == 'up':
            depth_offset = -1 * command.offset
        else:
            depth_offset = command.offset

        return Point(
            current.forward + forward_offset,
            current.depth + depth_offset,
        )

    location = Point(forward=0, depth=0)
    for string in strings:
        location = _move(location, _parse(string))

    return location.forward * location.depth
...
def second_task(strings: Iterable[str]) -> int:
    ...
    def _move(current: Point, command: Command):
        ...
        forward_offset = depth_offset = aim_offset = 0
        if command.action == 'forward':
            forward_offset = command.offset
            depth_offset = command.offset * current.aim
        elif command.action == 'up':
            aim_offset = -1 * command.offset
        else:
            aim_offset = command.offset

        return Point(
            current.forward + forward_offset,
            current.depth + depth_offset,
            current.aim + aim_offset,
        )
